...
text = path.read_text()
if 'execute_frame_phase17_bit7_index5' in text:
    raise SystemExit('index5 already present')

marker = 'static vf2_status execute_frame_phase17_bit7_index11(\n'
helper = r'''static vf2_status execute_frame_phase17_bit7_index5(
    vf2_model2a *machine,
    vf2_i960_cpu *cpu,
...
)
{
    ...
    if (status != VF2_OK || indirect_target != UINT32_C(0x0005b558) ||
        ...
        phase_a5 != 0u || phase_a6 != UINT8_C(0xff) || phase_a7 != UINT8_C(0xff) ||
        coin_flags != 0u || preset != 0u) {
    ...
    if (status == VF2_OK) {
        status = write_u16(machine, UINT32_C(0x01001320), UINT16_C(0x801c));
    }
    if (status == VF2_OK) {
    ...
    cpu->executed_instructions += UINT64_C(4188);
    cpu->procedure_calls += UINT64_C(35);
    cpu->procedure_returns += UINT64_C(35);
    ...
    report->recovered_instruction_count = UINT64_C(4188);
    report->recovered_procedure_calls = UINT64_C(35);
    report->recovered_procedure_returns = UINT64_C(36);
...
pattern = re.compile(r'(\s*if \(phase_index == UINT8_C\(0x84\)\) \{\s*return execute_frame_phase17_bit7_index4\(\s*machine, cpu, report, phase_index\s*\);\s*\}\s*)')
m = pattern.search(text)
if not m:
    raise SystemExit('index4 dispatch block not found')
insert = m.group(1) + '''        if (phase_index == UINT8_C(0x85)) {\n            return execute_frame_phase17_bit7_index5(\n                machine, cpu, report, phase_index\n            );\n        }\n'''
