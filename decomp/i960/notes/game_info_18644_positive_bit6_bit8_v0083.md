# `fa_game_info` `0x18644`: positive state-8 bit-6/bit-14/bit-21 composition

The measured positive-threshold fighter fields `0x204140` (state bit 8 plus
fighter flag bits 6, 14 and 21) were recovered at the `0x18644` child. The
second-order guard, bilateral admission and return-body accounting now cover
the isolated and bilateral compositions, including the measured mode-bit-6
and countdown joins.
...
python3 decomp/i960/tools/validate_game_info_state4.py \
  ...
  --state 8 --include-bit8 --extra-bit 6 --extra-bit 14 --extra-bit 21 \
