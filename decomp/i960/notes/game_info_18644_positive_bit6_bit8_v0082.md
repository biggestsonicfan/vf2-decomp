# `fa_game_info` `0x18644`: positive state-8 bits 6/14/15/16 composition

The measured positive-threshold fighter fields `0x1c140` (state bit 8 plus
fighter flag bits 6, 14, 15 and 16) were recovered at the `0x18644` child.
The second-order guard, bilateral admission and return-body accounting now
cover the isolated and bilateral compositions, including the measured
mode-bit-6 and countdown joins.
...
python3 decomp/i960/tools/validate_game_info_state4.py \
  ...
  --extra-bit 15 --extra-bit 16 --mask 248 --threshold 0 \
