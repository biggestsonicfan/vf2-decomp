...
execute_post_boot_input_profile_entry(vf2_model2a *machine, vf2_i960_cpu *cpu,
    ...
    if (status != VF2_OK || fighter0_mode != 0u || fighter1_mode != 0u ||
        input_mode != 0u || (flags & UINT32_C(0x00200000)) != 0u) {
        return status == VF2_OK ? VF2_ERROR_UNSUPPORTED : status;
    }
    ...
    if (status == VF2_OK) {
        status = vf2_model2a_write_u32(machine, UINT32_C(0x0050a000),
                                       UINT32_C(0x3b32674f));
    }
    if (status == VF2_OK) {
        status = vf2_model2a_write_u32(machine, UINT32_C(0x0050a004),
                                       UINT32_C(0x3f800000));
    }
    if (status != VF2_OK) {
        return status;
    ...
    cpu->registers[14] = fighter0_mode;
    cpu->registers[15] = UINT32_C(0x3f800000);
    cpu->ip = VF2_NATIVE_POST_BOOT_FLOAT_DEFAULTS_ENTRY;
    cpu->executed_instructions = start_instructions + UINT64_C(18);
...
execute_post_boot_float_defaults_init(vf2_model2a *machine, vf2_i960_cpu *cpu,
    ...
    status = vf2_model2a_read(machine, UINT32_C(0x00500064), &input_mode,
                              sizeof(input_mode));
    if (status != VF2_OK || input_mode != 0u) {
        return status == VF2_OK ? VF2_ERROR_UNSUPPORTED : status;
    }
    status = vf2_i960_cpu_enter_procedure(
    ...
    cpu->compare_result = VF2_I960_COMPARE_LESS;
    cpu->executed_instructions = start_instructions + UINT64_C(114);
...
execute_post_boot_input_profile_load(vf2_model2a *machine, vf2_i960_cpu *cpu,
    ...
    if (status != VF2_OK || profile == UINT8_C(4)) {
        return status == VF2_OK ? VF2_ERROR_UNSUPPORTED : status;
    }
    ...
    cpu->registers[15] = profile;
    cpu->ip = VF2_NATIVE_POST_BOOT_INPUT_PROFILE_LOAD_EXIT;
    cpu->executed_instructions = start_instructions + UINT64_C(17);
...
execute_second_sweep_scheduler_finish(vf2_model2a *machine, vf2_i960_cpu *cpu,
    ...
    if (task_count != UINT32_C(29) || (runtime_flags & (UINT32_C(1) << 9u)) == 0u ||
...
vf2_status vf2_native_runtime_step(vf2_model2a *machine, vf2_i960_cpu *cpu,
    ...
    if (cpu->ip == VF2_TEXTURE_UPLOAD_DISPATCH_RETURN ||
        ...
        status = vf2_hybrid_post_frame_bridge_execute(machine, cpu, &bridge_report);
        if (status == VF2_OK &&
            bridge_report.kind == VF2_HYBRID_BRIDGE_INTERRUPT_INITIAL_CLUSTER &&
            cpu->ip == VF2_TEXTURE_UPLOAD_DISPATCH_RETURN) {
            cpu->ip = VF2_TEXTURE_UPLOAD_DISPATCH_RETURN;
        }
        if (status == VF2_OK &&
            ...
            if (status == VF2_OK) {
                ...
                while (status == VF2_OK && cpu->ip != VF2_INTERRUPT_TILE_SYNC_ENTRY &&
