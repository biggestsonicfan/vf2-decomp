...
static void test_invalid_arguments(void)
{
    ...
    CHECK(
        vf2_native_differential_run_cycles(
            NULL,
            NULL,
            NULL,
            NULL,
...
static void test_zero_cycle_match(void)
{
    vf2_model2a reference_machine;
    vf2_model2a native_machine;
    vf2_i960_cpu reference_cpu;
...
static void test_cycle_contract_is_enforced(void)
{
    vf2_model2a reference_machine;
    vf2_model2a native_machine;
    vf2_i960_cpu reference_cpu;
    ...
    CHECK(report.completed == 0);
    CHECK(report.requested_cycles == 2u);
    CHECK(report.completed_cycles == 0u);
    CHECK(report.blocks_compared == 0u);
    CHECK(report.reference_instructions_executed == 0u);
    CHECK(report.native_recovered_instructions == 0u);
    CHECK(report.last_cycle.minimum_blocks == 1u);
    CHECK(report.last_cycle.last_step.entry_address == UINT32_C(0xdeadbeef));
    CHECK(report.final_reference_address == UINT32_C(0xdeadbeef));
    CHECK(report.final_native_address == UINT32_C(0xdeadbeef));
