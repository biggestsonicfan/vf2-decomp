...
text = p.read_text()
needle = "vf2_status execute_display_color_profile_apply(\n"
if text.count(needle) != 1:
    raise SystemExit(f"video insertion needle count: {text.count(needle)}")
p.write_text(text.replace(needle, function + needle, 1))

Path("decomp/i960/notes/display_profile_unit_fill_executable_v0026.md").write_text(r'''# Executable display profile unit fill (v0.0.26)
