...
| Fighter-state bit-31 runtime path | Native dispatcher; observed `0x18144` corridors through `0x18d44`, `0x18c64` and `0x18640`; controlled `0x181c0` bodies, observed state-4/bit-15 and non-state-4 bit-15 prefixes, shared `0x18e08`/`0x18e00` helpers and observed bit-14/15/16 dependent `0x18644` flag paths are native, while unobserved downstream branches and `fa_player` remain explicit ROM-backed boundaries |
