...
| State-8 ten-bit matrix | Bits 6/21/26/29/30/31 plus bits 1/2/4/8 are exact at threshold `-1`: 12,288 fixtures (1,024 masks × 12 distributions). The nine-bit subset without bit 6 is exact at threshold `0`; positive bit 6 is additionally exact for the complete no-bit-8 bits-1/2/4/6 submatrix (192 fixtures) and eight full-high masks with bit 8 covering every subset of low bits 1/2/4 (96 more fixtures). Other positive bit-6 compositions remain explicit unsupported boundaries. |
