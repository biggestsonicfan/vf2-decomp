...
)
{
    ...
    if (status == VF2_OK) {
        status = vf2_i960_snapshot_read_file(&snapshot, temporary_snapshot);
    }
    if (status == VF2_OK) {
    ...
    if (status == VF2_OK) {
        status = vf2_native_runtime_state_read_file(
            &runtime_state, temporary_runtime
        );
    }
    if (status != VF2_OK) {
        fprintf(
            stderr,
            "Could not restore sixth-dispatch checkpoint: %s\n",
    ...
    for (dispatch = 7u; status == VF2_OK && dispatch <= dispatch_count; ++dispatch) {
        memset(&report, 0, sizeof(report));
    ...
    if (status == VF2_OK && output_snapshot_path != NULL) {
        ...
        if (status == VF2_OK) {
            printf("Native dispatch %u snapshot:          %s\n",
                   (unsigned)dispatch_count, output_snapshot_path);
        }
    }
...
runtime_cleanup:
    ...
    free(image);
    result = status == VF2_OK ? EXIT_SUCCESS : EXIT_FAILURE;
