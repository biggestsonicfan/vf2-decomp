...
bridge_path = Path("src/recovered/texture_bridge.c")
bridge = bridge_path.read_text(encoding="utf-8")
replacement = r'''static vf2_status execute_texture_status_dispatch_call(
    vf2_model2a *machine,
    vf2_i960_cpu *cpu,
...
pattern = re.compile(
    r"static vf2_status execute_texture_status_dispatch_call\(.*?"
    r"static vf2_status execute_texture_final_status_call\(",
    re.DOTALL,
)
bridge, count = pattern.subn(replacement, bridge, count=1)
if count != 1:
    raise SystemExit(
        f"status dispatch replacement: expected one, found {count}"
    )
...
replace_once(
    ...
    "             bridge_recovered_instructions != UINT64_C(1268909) ||\n"
    "             bridge_interpreted_instructions != UINT64_C(1913) ||\n"
    "             bridge_validated_blocks != 155u ||\n"
    "             bridge_memory_checkpoints != 155u ||\n",
    "strict totals",
)
replace_once(
    "tools/vf2i960/main.c",
...
test_path = Path("tests/analysis/test_orchestrator_limits.c")
tests = test_path.read_text(encoding="utf-8")
test_function = r'''static void test_inactive_record_scan(void)
{
    vf2_model2a machine;
    vf2_i960_cpu cpu;
    vf2_hybrid_bridge_report report = {0};
    uint8_t inactive[2] = {0u, 0u};
    size_t index = 0u;
    ...
    CHECK(report.entry_address == UINT32_C(0x0004bd24));
    CHECK(report.exit_address == UINT32_C(0x0004bf90));
    ...
    CHECK(cpu.ip == UINT32_C(0x0004bf90));
    ...
    CHECK(cpu.registers[5] == UINT32_C(0x005502a8));
    CHECK(cpu.registers[6] == UINT32_C(0x005502a8));
...
'''
if tests.count("int main(void)\n{") != 1:
    raise SystemExit("test main marker not unique")
tests = tests.replace(
    "int main(void)\n{",
    test_function + "int main(void)\n{",
    1,
)
if tests.count("    test_hybrid_bridge_poststate();\n") != 1:
    raise SystemExit("test invocation marker not unique")
tests = tests.replace(
    "    test_hybrid_bridge_poststate();\n",
    "    test_hybrid_bridge_poststate();\n"
    "    test_inactive_record_scan();\n",
    1,
)
...
replace_once(
    ...
    "recovered-observed-branch,dynamic-trace+unit,"
    "Four observed record dispatches recover eight instructions each\n"
    "0x0004bd24,0x0004bf90,texture_status_scan_end,"
    "recovered-observed-branch,dynamic-trace+unit,"
    "Final pass scans ten inactive records and branches to final status\n",
    "function catalog scan entry",
)
...
- `0x0004bd24`: four 8-instruction active-record dispatches;
- `0x0004bd24`: one 43-instruction inactive-record scan;
...
These blocks account for 157 recovered instructions in the observed orchestrator path. Unsupported branch classes continue to return `VF2_ERROR_UNSUPPORTED` rather than guessing.
...
- 1,268,909 recovered instructions;
- 1,913 interpreted instructions;
- 155 recovered blocks and memory checkpoints;
